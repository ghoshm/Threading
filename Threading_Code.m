...
c = nan(max(experiment_reps),1,'single'); % experiemtn groups x 1 
...
time_bins = fps{1}*60*5; % set smoothing window (note assumes a constant frame rate)
...
for er = 1:max(experiment_reps)
    fish_tags_cm(er,1) = size(raw_data{er,1},1);
end
fish_tags_cm = cumsum(fish_tags_cm);

% Allocate experiment reps to every active bout 
experiment_reps_long{1,1} = experiment_tags{1,1};
...
for c = find(ai_states == 1) % for each active bout type
    ...
    for b = 1:number % for each bout
        ...
        er = sample_er(sample_tags(b,1));
        if er == 1
            fish = sample_fish(sample_tags(b,1));
        else
            fish = sample_fish(sample_tags(b,1)) - fish_tags_cm(er - 1); 
        end
        
        et = find(find(experiment_reps == er) == sample_et(sample_tags(b,1)));
...
axis([0 8/fps{1} ylim]);
set(gca,'XTick',(1/fps{1}):(2/fps{1}):(10/fps{1}));
set(gca,'XTickLabels',{(round((1/fps{1}):(2/fps{1}):(10/fps{1}),2,'decimals'))});
xlabel('Time (seconds)','Fontsize',32);
ylabel('Delta Px','Fontsize',32);
[~,icons,plots,~] = legend(legend_cols,legend_cell,'Location','northeast');
legend('boxoff'); set(icons(1:size(bouts,2)),'Fontsize',18) ; set(plots,'LineWidth',2);
